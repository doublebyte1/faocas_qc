  ...
  NAME_LOOP:
	...
	if ( $name =~ m,^([^/]+\.framework)/(\S+)$, ) {
		...
		if ( ! -e $fwPath ) {
		    ...
		    `$c`;
			`rm -fr $frameworks_dir/$fwname/*.dSYM`;
			`rm -fr $frameworks_dir/$fwname/*_debug`;
			`rm -fr $frameworks_dir/$fwname/Headers`;
			`rm -fr $frameworks_dir/$fwname/Versions/4/*_debug`;
			`rm -fr $frameworks_dir/$fwname/Versions/4/Headers`;
		} else {
		    $not_existed = 0;
		}
